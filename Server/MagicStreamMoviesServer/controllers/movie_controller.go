...
import (
	...
	"github.com/cuongasd52/MagicStreamMovies/Server/MagicStreamMoviesServer/database"
	"github.com/cuongasd52/MagicStreamMovies/Server/MagicStreamMoviesServer/models"
	"github.com/cuongasd52/MagicStreamMovies/Server/MagicStreamMoviesServer/utils"
...
func GetMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error while fetching movies from database"})
			return
		}
...
func AdminReviewUpdate(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Printf("Error from GetReviewRanking: %v\n", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error while getting review ranking"})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error while updating movie review"})
			return
		}
...
func GetReviewRanking(admin_review string, client *mongo.Client, c *gin.Context) (string, int, error) {
	...
	for _, ranking := range rankings {
		if ranking.RankingValue != 999 {
			sentimentDelimeted += ranking.RankingName + ", "
		}
	}
	...
	if GoogleAiApiKey == "" {
		return "", 0, errors.New("GOOGLE_API_KEY not set in .env file")
	}
	...
	response, err := llm.Call(context.Background(), base_prompt+"\n"+admin_review)
	if err != nil {
		return "", 0, err
	}
	rankVal := 0
...
func GetRecommendedMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error while fetching user favorite genres"})
			return
		}
		err = godotenv.Load(".env")
		if err != nil {
